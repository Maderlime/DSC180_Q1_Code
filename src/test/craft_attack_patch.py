...
print("we were able to do the imports correctly....")
DATA_PATH = "././test/testdata/"
class Dermatology_image_loader(object):
    def __init__(self):
        self.X_test = np.load(DATA_PATH + 'data/test_x_preprocess_sample.npy')
        self.y_test = np.load(DATA_PATH + 'data/test_y_sample.npy')
        self.X_train = np.load(DATA_PATH + 'data/train_x_preprocess_sample.npy')
        self.y_train = np.load(DATA_PATH + 'data/train_y_sample.npy')
...
class ModelContainer():
    ...
    def _make_model_and_ops(self, M, keras_mode, patch_val, verbose):
        ...
        with self.sess.graph.as_default():
            ...
            print("#############################")
            model = keras.models.load_model((DATA_PATH + 'models/wb_model.h5').encode('utf-8'))
...
def attack(model, patch, target_label, n_show=5, scale=0.5, show_indices=None,
    ...
    if predict_original:
        ...
        probs_original_images = np.load(DATA_PATH + './etc_saved_files/' + file_name)
...
def load_craft_attack(): # TARGET_LABEL, PATCH_SHAPE, BATCH_SIZE, DATA_PATH
